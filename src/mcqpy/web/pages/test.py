...
def _process_image_captions(image_caption: dict, num_images: int):
    ...
    sub_captions = [
        caption
        for index, caption in image_caption.items()
        if index >= 0
    ]
...
def render_question(question: Question, index: int = 1):
    with st.container(border=True):
        ...
        if question.image:
            ...
            sub_captions = _process_image_captions(question.image_caption, len(question.image))
            
            # Use columns for better image loading performance
            if len(question.image) == 1:
                st.image(question.image[0], width=400, caption=sub_captions[0] if sub_captions else None)
            else:
...
def grade_quiz(questions: list[Question], answers: dict = None):
    ...
    for question in questions:
        ...
        if question_answers is None:
            parsed_questions.append(ParsedQuestion(qid=qid, slug=question.slug, answers=None, onehot=[0]*len(question.choices) ))
        ...
        if isinstance(question_answers, str):
            ...
            parsed_questions.append(
                ParsedQuestion(qid=qid, slug=question.slug, answers=[index], onehot=onehot)
            )
        elif isinstance(question_answers, list):
            # Multiple choice
            indices = [ord(a[0]) - 65 for a in question_answers]  # Convert 'A' to 0, 'B' to 1, etc.
            onehot = [1 if i in indices else 0 for i in range(len(question.choices))]
            parsed_questions.append(
                ParsedQuestion(qid=qid, slug=question.slug, answers=indices, onehot=onehot)
            )
...
def _upload_questions(questions):
    import yaml
    uploaded_files = st.file_uploader("Upload Questions File", type=["yaml"], accept_multiple_files=True)
    if uploaded_files is not None:
        for uploaded_file in uploaded_files:
            data = yaml.safe_load(uploaded_file)
            question = Question.model_validate(data, context={"base_dir": Path.cwd() / "questions"})
            questions.append(question)

def _load_questions_from_link(link=None, questions=None):
    ...
    use_cache = st.checkbox(
        "🚀 Download images locally (faster navigation)", 
        value=False,
        help="Downloads images to temporary directory for faster question navigation. Slower initial load but much faster switching between questions."
    )
    
    if link:
        loading_msg = "Downloading questions and images..." if use_cache else "Loading questions from remote repository..."
        with st.spinner(loading_msg):
            quiz_content = load_remote_quiz(link, use_cache=use_cache)
            if quiz_content and 'questions' in quiz_content:
                questions.extend(quiz_content.get('questions', []))
                ...
                if quiz_content.get('loader_type') == 'cached':
                    st.success(f"🚀 Successfully downloaded {num_questions} questions and images to local cache!")
                    st.info(f"📁 Cache directory: `{quiz_content.get('cache_dir', 'Unknown')}`")
                else:
...
def _load_questions_with_token(token=None, questions=None):
    ...
    if token:
        ...
        st.info(f"Loading questions with token {token} is not yet implemented.")

# Upload questions:
questions = []
expander = st.expander("Load Questions", expanded=True)
...
if len(questions) == 0:
    with expander:
        ...
        with question_loader:
            ...
            upload_tab, link_tab, token_tab = st.tabs(["📁  Upload Questions", "🔗  Load link", "🔑  Load with token"])

            with upload_tab:
               _upload_questions(questions=questions)

            with link_tab:
                _load_questions_from_link(questions=questions)

            with token_tab:
                _load_questions_with_token(questions=questions)

        if len(questions) > 0:
            for question in questions:
                st.success(f"Loaded question: {question.slug} (QID: {question.qid})")
...
if len(questions) != 0:
    ...
    with col1:
        prev_clicked = st.button("⬅️ Previous Question", width="stretch", key='prev_question_button')
        
    with col3:
        next_clicked = st.button("Next Question ➡️", width="stretch", key='next_question_button')

    # Update session state after capturing button states
    if prev_clicked:
        st.session_state.quiz_page_idx = max(0, st.session_state.quiz_page_idx - 1)
    if next_clicked:
        st.session_state.quiz_page_idx = min(n_pages-1, st.session_state.quiz_page_idx + 1)

    add_shortcuts(prev_question_button='arrowleft', 
                  next_question_button='arrowright')
    ...
    with col2:
        ...
        st.number_input("Jump to Question", min_value=0, max_value=n_pages-1, key="quiz_page_idx")

    if 1 <= st.session_state.quiz_page_idx <= len(questions):
        render_question(
            question=questions[st.session_state.quiz_page_idx - 1],
        ...
        for idx, question in enumerate(questions, start=1):
            answer = st.session_state.quiz_answers.get(question.qid, "No answer selected")
            st.markdown(f"- Question {idx}: Your answer: {answer}")

        if st.button("Grade Quiz", key="grade_quiz_button"):
            graded = grade_quiz(questions=questions, answers=st.session_state.quiz_answers)
            st.subheader("Grading Results")

            per_question_points = [g.point_value for g in graded.graded_questions]

            df = pd.DataFrame({
                "Question": [f"Q{idx+1}" for idx in range(len(graded.graded_questions))],
            ...
            with column1:
                st.metric(
                    label="Total Score",
                    value=f"{graded.points} / {graded.max_points}"
                )
