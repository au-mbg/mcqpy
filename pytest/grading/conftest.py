from mcqpy_core.grading import (
    GradedSet,
    GradeResult,
    GradeState,
    StrictRubric,
...
class PDFPipelineGrader:
    ...
    def grade(self, student_answer=None, parsed_set=None) -> GradeResult:
        if parsed_set is None:
            parsed_result = self.parser.parse_pdf(
                student_answer, regex_pattern=self.regex_pattern
            )
            parsed_set = parsed_result.parsed_set
        grade_result = grade_parsed_set(self.manifest, self.rubric, parsed_set)
        return grade_result
...
def graded_set(request, grader, filled_pdfs) -> GradedSet:
    grade_result = grader.grade(filled_pdfs[request.param])    
    return grade_result.graded_set

@pytest.fixture(scope="module")
def graded_sets(grader, filled_pdfs) -> list[GradedSet]:
    graded_sets = []
    for pdf in filled_pdfs:
        grade_result = grader.grade(pdf)
